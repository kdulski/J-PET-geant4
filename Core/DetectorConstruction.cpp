...
//      ConstructScintillators();
      ConstructScintillatorsLSO();
      ConstructTargetForImaging();
    }
...
void DetectorConstruction::InitializeMaterials()
{
  ...
  fLSO = new MaterialExtension(MaterialParameters::MaterialID::mScin, "Lu2SiO5", LSO);

  nistManager->FindOrBuildMaterial("G4_WATER");
  fWater = new MaterialExtension(MaterialParameters::MaterialID::mWater, "water", G4Material::GetMaterial("G4_WATER"));
  fWater->AllowsAnnihilations(true);
}
...
void DetectorConstruction::ConstructScintillatorsLSO()
{
  ...
  G4Material* default_mat = fAir;
  G4Material* cryst_mat = fLSO;

  G4Tubs* solidRing = new G4Tubs("Ring", ring_R1, ring_R2, 0.5*cryst_dX, 0., twopi);
  G4LogicalVolume* logicRing = new G4LogicalVolume(solidRing, default_mat, "Ring");
  ...
  fScinLog = new G4LogicalVolume(solidCryst, cryst_mat, "CrystalLV");
  ...
  for (G4int icrys=0; icrys<nb_cryst; icrys++) {
    ...
    new G4PVPlacement(transform, fScinLog, "crystal", logicRing, false, icrys, fCheckOverlaps);
  }

  G4Tubs* solidDetector = new G4Tubs("Detector", ring_R1, ring_R2, 0.5*detector_dZ, 0., twopi);
  G4LogicalVolume* logicDetector = new G4LogicalVolume(solidDetector, default_mat, "Detector");

  G4double OG = -0.5*(detector_dZ + cryst_dX);
  for (G4int iring=0; iring<nb_rings; iring++) {
    OG += cryst_dX;
    new G4PVPlacement(0, G4ThreeVector(0, 0, OG), logicRing, "ring", logicDetector, false, iring, fCheckOverlaps);
  }

  new G4PVPlacement(0, G4ThreeVector(), logicDetector, "Detector", fWorldLogical, false, 0, fCheckOverlaps);

  logicRing->SetVisAttributes(G4Colour(0.5, 0.5, 0.9));
}
...
void DetectorConstruction::ConstructTargetForImaging()
{
  G4double boxSizeX = 10.0*cm, boxSizeY = 10.0*cm, boxSizeZ = 10.0*cm;
  G4ThreeVector shiftFromCenter(0, 6.0*cm, 0);

  fImagingPlateSize = {boxSizeX, boxSizeY, boxSizeZ};
  fImagingPlatePositions.push_back(G4ThreeVector(0, 6.0*cm, 0));
  fImagingPlatePositions.push_back(G4ThreeVector(0, -6.0*cm, 0));

  G4Box* solidBox = new G4Box("solidBox", boxSizeX/2, boxSizeY/2, boxSizeZ/2);
  G4LogicalVolume* logicBox1 = new G4LogicalVolume(solidBox, fPlexiglass, "box1");
  G4LogicalVolume* logicBox2 = new G4LogicalVolume(solidBox, fWater, "box2");
  G4VPhysicalVolume* physiBox1 = new G4PVPlacement(0, shiftFromCenter, logicBox1, "box1Placement", fWorldLogical, true, 0, checkOverlaps);
  G4VPhysicalVolume* physiBox2 = new G4PVPlacement(0, -shiftFromCenter, logicBox2, "box2Placement", fWorldLogical, true, 0, checkOverlaps);

  G4VisAttributes* boxVisAtt1 = new G4VisAttributes(G4Colour(0.9, 0.9, 0.9));
  boxVisAtt1->SetForceWireframe(true);
  boxVisAtt1->SetForceSolid(true);
  logicBox1->SetVisAttributes(boxVisAtt1);
  G4VisAttributes* boxVisAtt2 = new G4VisAttributes(G4Colour(0.5, 0.8, 0.2));
  boxVisAtt2->SetForceWireframe(true);
  boxVisAtt2->SetForceSolid(true);
  logicBox2->SetVisAttributes(boxVisAtt2);
}
