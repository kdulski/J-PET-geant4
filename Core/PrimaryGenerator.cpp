...
void PrimaryGenerator::GenerateEvtImaging(G4Event* event)
{
  G4ThreeVector platSize = DetectorConstruction::GetInstance()->getImagingPlateSize();
  std::vector<G4ThreeVector> platPos = DetectorConstruction::GetInstance()->getImagingPlatePositions();

  G4double random = G4UniformRand() - 0.5;
  G4double random2 = G4UniformRand() - 0.5;
  G4double random3 = G4UniformRand() - 0.5;
  G4double random4 = G4UniformRand();

  double increment = 1.0/platPos.size();
  int it = -1;
  ...
  G4ThreeVector chosenPlatPos = platPos.at(it);
  G4ThreeVector vtxPosition = {random*platSize.getX() + chosenPlatPos.getX(),
                               random2*platSize.getY() + chosenPlatPos.getY(),
                               random3*platSize.getZ() + chosenPlatPos.getZ()};

  MaterialExtension* material = dynamic_cast<MaterialExtension*>(theNavigator->LocateGlobalPointAndSetup(vtxPosition)->GetLogicalVolume()->GetMaterial());
  std::vector<G4double> evtFractions = material->GetEventsFraction();
  G4double random5 = G4UniformRand();

  G4double T0 = 0;

  if (evtFractions[0] > random5) {
    // pPs 2G
    event->AddPrimaryVertex(GenerateTwoGammaVertex(
      vtxPosition, T0,
      material->GetLifetime(random5, DecayChannel::kPara2G)
    ));
    fDecayChannel = DecayChannel::kPara2G;
  } else if (evtFractions[0] + evtFractions[1] > random5) {
    // Direct 2G
    event->AddPrimaryVertex(GenerateTwoGammaVertex(
      vtxPosition, T0,
      material->GetLifetime(random5 - evtFractions[0], DecayChannel::kDirect2G)
    ));
    fDecayChannel = DecayChannel::kDirect2G;
  } else if (evtFractions[0] + evtFractions[1] + evtFractions[2] > random5) {
    // oPs 2G
    event->AddPrimaryVertex(GenerateTwoGammaVertex(
      vtxPosition, T0,
      material->GetLifetime(random5 - evtFractions[0] - evtFractions[1], DecayChannel::kOrtho2G)
    ));
    fDecayChannel = DecayChannel::kOrtho2G;
  } else if (evtFractions[0] + evtFractions[1] + evtFractions[2] + evtFractions[3] > random5) {
    // pPs 3G
    event->AddPrimaryVertex(GenerateThreeGammaVertex(
      DecayChannel::kPara3G, vtxPosition, T0,
      material->GetLifetime(random5 - evtFractions[0] - evtFractions[1] - evtFractions[2], DecayChannel::kPara3G)
    ));
    fDecayChannel = DecayChannel::kPara3G;
  } else if (evtFractions[0] + evtFractions[1] + evtFractions[2] + evtFractions[3] + evtFractions[4] > random5) {
    // Direct 3G
    event->AddPrimaryVertex(GenerateThreeGammaVertex(
      DecayChannel::kDirect3G, vtxPosition, T0,
      material->GetLifetime(random5 - evtFractions[0] - evtFractions[1] - evtFractions[2] - evtFractions[3], DecayChannel::kDirect3G)
    ));
    fDecayChannel = DecayChannel::kDirect3G;
  } else {
    // oPs 3G
    event->AddPrimaryVertex(GenerateThreeGammaVertex(
      DecayChannel::kOrtho3G, vtxPosition, T0,
      material->GetLifetime(random5 - evtFractions[0] - evtFractions[1] - evtFractions[2] - evtFractions[3] - evtFractions[4], DecayChannel::kOrtho3G)
    ));
    fDecayChannel = DecayChannel::kOrtho3G;
  }

  G4ThreeVector promptVtxPosition = vtxPosition;
  event->AddPrimaryVertex(GeneratePromptGammaVertex(
    promptVtxPosition, 0.0f, MaterialParameters::fSodiumGammaTau,
    MaterialParameters::fSodiumGammaEnergy
  ));
